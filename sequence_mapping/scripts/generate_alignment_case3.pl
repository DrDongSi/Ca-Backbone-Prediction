...
## perl /storage/htc/bdm/jh7x3/Cryo_em_paper/paper_version_20190817/New_sequence_mapping_jiealgo/scripts/generate_alignment_case3.pl /storage/htc/bdm/jh7x3/Cryo_em_paper/paper_version_20190817/New_sequence_mapping_jiealgo/7proteins/3i2n_3A/test_results_merge/frag_merge_dir/frag_merged1.pdb /storage/htc/bdm/jh7x3/Cryo_em_paper/paper_version_20190817/New_sequence_mapping_jiealgo/7proteins/fastas_final/3i2n.fasta   /storage/htc/bdm/jh7x3/Cryo_em_paper/paper_version_20190817/New_sequence_mapping_jiealgo/7proteins/3i2n_3A/test_results_merge/sampling 10 /storage/htc/bdm/jh7x3/Cryo_em_paper/paper_version_20190817/New_sequence_mapping_jiealgo/7proteins/3i2n_3A/test_results_merge/frag_merge_dir/frag_merged1/sample.regions
...
$proc_num = shift @ARGV;
$sample_regions = shift @ARGV; #144-148:11.1472111758951

open(TMP,"$sample_regions") || die "Failed to open file $sample_regions\n";
@regions = ();
while(<TMP>)
{
  $line=$_;
...
for($epoch=1;$epoch<=100;$epoch++)
{
  ...
  $newseq = $pdb_seq;
  print "Insert $n_gaps gaps in position";
  @sample_positions = ();
  foreach (@regions)
  {
    @tmp2 = split('-',$_);
    ...
    for($i=$start;$i<=$end;$i++)
    {
      push @sample_positions,$i-1;
    }
  }
  push @sample_positions,0;
  push @sample_positions,length($newseq)-1;
  @array = shuffle(@sample_positions);
  for($i=0;$i<$n_gaps;$i++)
  {
    $idx = 0 + int(rand(@array));
    $pos = $array[$idx];
    print " $pos";
